...
from nimbro_api_interfaces.srv import TranscriptionsGet
...
api_endpoints = {
    ...
    'vLLM': {
        ...
        'transcriptions_url': "http://localhost:8000/v1/audio/transcriptions",
        'key_type': "environment",
    ...
    'AIS': {
        ...
        'transcriptions_url': "https://api-code.ais.uni-bonn.de/v1/audio/transcriptions",
        'key_type': "environment",
...
class Transcriptions(Node):
    ...
    def __init__(self, name=node_name, *, context=None, **kwargs):
        ...
        self.endpoint_keys_required = {'name', 'api_flavor', 'transcriptions_url', 'key_type', 'key_value'}
        ...
        self.parameter_handler.declare(
            ...
            description=f"Sets the API endpoint defining API flavor, Models & Transcriptions URLs, key type and value. Must be a valid JSON encoded dictionary or a name in {list(api_endpoints.keys())}.",
            read_only=False
        )
        ...
        self.cbg_transcriptions = ReentrantCallbackGroup()
        self.srv_transcriptions = self.create_service(TranscriptionsGet, f"{self.node_namespace}/{self.node_name}/get_transcription".replace("//", "/"), self.get_transcription_callback, qos_profile=qos_profile, callback_group=self.cbg_transcriptions)
    ...
    def transcriptions_post(self, path, model, temperature, language, prompt, response_format, api_url, api_key):
        self._logger.debug(f"Retrieving transcription from API (path='{path}', model='{model}', temperature='{temperature}', language='{language}', prompt='{prompt}', response_format='{response_format}')")
        ...
        if response.status_code == 200:
            ...
            try:
                transcription = json.dumps(response.json(), indent=2)
            except Exception:
        ...
        return success, message, transcription

    def get_transcription(self, path, model, temperature, language, prompt, response_format):
        # parse arguments
        ...
        supported_formats = ["json", "verbose_json", "diarized_json", "text", "srt", "vtt"]
        if response_format == "":
        ...
        success, message, transcription = self.transcriptions_post(
            ...
            prompt=prompt,
            response_format=response_format,
            api_url=self.api_endpoints[self.parameters.api_endpoint]['transcriptions_url'],
            api_key=api_key
        )
        if not success:
            log_lines(
        ...
        log_lines(
            text=f"Retrieved transcription (path='{path}', model='{model}', temperature='{temperature}', language='{language}', prompt='{prompt}', response_format='{response_format}'):\n{transcription}",
            line_length=self.parameters.log_line_length,
            line_highlight="| ",
            block_format=False,
            logger=self._logger,
        ...
        return True, "Retrieved transcription.", transcription

    # Callbacks

    def get_transcription_callback(self, request, response):
        self._logger.debug("get_transcription_callback(): start")

        # chunking_strategy, known_speaker_names
        response.success, response.message, transcription = self.get_transcription(
            path=request.path,
            model=request.model,
        ...
        self._logger.debug("get_transcription_callback(): end")
        return response

def main(args=None):
    start_and_spin_node(Transcriptions, args=args)
